...
class EscPosPrinterService
{
    ...
    public static function printKotDirect(Kot $kot, Printer $printer): bool
    {
        ...
        try {
            ...
            if ($printerName !== '') {
                return self::sendToCompanion($printerName, $bytes, $kot->id, $kot->branch_id);
            }
    ...
    /**
     * Send raw ESC/POS payload to GenX Print Companion (Local Agent or Cloud Bridge Relay)
     */
    public static function sendToCompanion(string $printerName, string $bytes, int $kotId = 0, int $branchId = 0): bool
    {
        ...
        try {
            ...
            if ($httpCode === 200) {
                Log::info("EscPosPrinterService: Successfully dispatched KOT via Local Companion to printer: {$printerName}");
                return true;
            }
        } catch (\Throwable $e) {
